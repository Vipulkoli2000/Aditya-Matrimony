...
<div>
    ...
    <div class="pt-5">        
        <form class="space-y-5" action="{{ route('castes.update', ['caste' => $caste->id]) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="panel">
                <div class="flex items-center justify-between mb-5">
                    <h5 class="font-semibold text-lg dark:text-white-light">Edit Caste</h5>
                </div>
                <div class="grid grid-cols-1 gap-4 mb-4 md:grid-cols-3">               
                    <x-text-input name="name" value="{{ old('name', $caste->name) }}" :label="__('Caste Name')" :require="true" :messages="$errors->get('name')"/>              
                </div>
                        ...
                        @if(old('subcastes'))
                            @foreach(old('subcastes') as $index => $subcaste)
                                <div class="subcaste-row flex gap-2 mb-2">
                                    <input type="hidden" name="subcaste_ids[]" value="{{ old('subcaste_ids')[$index] ?? '' }}">
                            ...
                            @endforeach
                        @else
                            @forelse($caste->subCastes as $subcaste)
                                <div class="subcaste-row flex gap-2 mb-2">
                                    <input type="hidden" name="subcaste_ids[]" value="{{ $subcaste->id }}">
                                    <input type="text" name="subcastes[]" value="{{ $subcaste->name }}" class="form-input flex-1" placeholder="Enter subcaste name" required>
                                    <button type="button" class="btn btn-danger remove-subcaste">Remove</button>
                                </div>
                            @empty
                                <div class="subcaste-row flex gap-2 mb-2">
                                    <input type="hidden" name="subcaste_ids[]" value="">
